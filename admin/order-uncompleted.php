        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4">
              <span class="text-muted fw-light">Services / Shippments / </span> Uncompleted Orders
            </h4>
            ...
            <div class="card">
              <div class="card-header border-bottom">
                <h5 class="card-title">Uncompleted Orders</h5>
              </div>
              <div class="card-datatable table-responsive text-nowrap">
                <table class="datatables-users table border-top" id="example1">
                  ...
                  <tbody class="table-border-bottom-0">
                    ...
                    $conn = $pdo->open();
                    try{
                      $stmt = $conn->prepare("SELECT * FROM orders WHERE status = 7 ORDER BY date_created DESC");
                      $stmt->execute();
                      $i = 0;
                      foreach($stmt as $row){
                        if ($row['status'] == 0) {
                        ...
                        if ($currency['slug'] == "Yuan") {
                          if ($row['coupon_code'] !== "") {
                            $amount = "<div class='text-muted lh-1'><span class='text-primary fw-semibold'>".$currency['sign']."".number_format($grand_total_yuan)."</span></div>
                          ...
                          if ($row['coupon_code'] !== "") {
                            $amount = "<div class='text-muted lh-1'><span class='text-primary fw-semibold'>".$currency['sign']."".number_format($grand_total_usd)."</span></div>
                            <small class='text-muted'>".$settings['currency']."".number_format($grand_total_naira)."</small>";
                          }else {
                            $amount = "<div class='text-muted lh-1'><span class='text-primary fw-semibold'>".$currency['sign']."".number_format($total_usd)."</span></div>
                            <small class='text-muted'>".$settings['currency']."".$total_naira."</small>";
                          }
                        }
