<?php
	...
	if(isset($_POST['save'])){
		...
		// if(password_verify($curr_password, $admin['password'])){
			...
			try{
				$stmt = $conn->prepare("UPDATE drivers SET main_bank_name=:main_bank_name, main_account_number=:main_account_number, name_on_account=:name_on_account, short_code=:short_code WHERE id=:id");
