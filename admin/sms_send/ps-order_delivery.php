<?php
  ...
  $conn = $pdo->open();
  $sender_name = "SendASAP";
  $message = "Hello, this is to confirm that your Procuement & Shipping Order with ID ".$ref_id." is being Delivered to your location.";
  $real_message = $message;
  $encode_message = urlencode($message);
  $receipient = $iow['contact_info'];
  ...
  $text_dnd = $settings['text_api_dnd'];
    ...
    // echo "Link - https://www.bulksmsnigeria.com/api/v1/sms/create?api_token={$settings['text_api_key']}&from={$sender_name}&to={$receipient}&body={$encode_message}&dnd={$text_dnd} <br>";
    // echo "To - $receipient <br>";
